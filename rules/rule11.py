...
##################################################
### RULE 11:  lf_paper_triggers_200char_negate ###
##################################################
...
negate: keywords that indicate negation
NOT_MATCHING: any trigger word in paper_triggers within 200 characters of any keyword in negate found in discharge summary
'''
...
negate_raw = pd.read_csv("../rules/keywords/negate.csv")
negate = list(negate_raw['negate'])

@labeling_function()
# NOT_MATCHING: any trigger word in paper_triggers within 200 characters of any keyword in negate found in discharge summary
def lf_paper_triggers_200char_negate(x) :
    # set distance
    dist = 200
    found = 0
    for i in range(0, len(ade_drug_single)) :
        ...
        for j in range(0, len(pos)) :
            ...
            if start_pos < 0 :
                start_pos = 0
        if any(word in x.summary.lower()[start_pos:end_pos] for word in paper_triggers) and any(word in x.summary.lower()[start_pos:end_pos] for word in negate):
